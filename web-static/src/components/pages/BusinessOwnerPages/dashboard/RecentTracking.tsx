...
const CustomizedTables: React.FC<CustomizedTablesProps> = ({
  ...
  return (
    <div className="w-full">
        ...
        >
          ...
          >
            ...
            <select
              ...
              className={`outline-none ${isDarkMode
      ...
      {/* Table */}
      <div
        className={`overflow-x-auto rounded-lg max-h-[500px] overflow-y-auto ${isDarkMode
          ? 'border border-zinc-700 bg-zinc-900'
          : 'border border-gray-300 bg-white'
          }`}
      >
        <table
          className={`min-w-full text-left text-sm ${isDarkMode ? 'text-white' : 'text-black'
            }`}
        >
          <thead
            className={`${isDarkMode
              ? 'bg-zinc-900 text-gray-300'
              : 'bg-gray-100 text-[#537086]'
            ...
            {dataToShow.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <Link
                    to={`/dashboard/trackings/${item.parcel_number}/`}
                    state={item}
                    className={`
                      underline font-medium
                      ${isDarkMode
                        ? 'text-blue-400 visited:text-purple-400'
